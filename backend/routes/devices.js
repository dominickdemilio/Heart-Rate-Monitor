...
router.put('/update/:deviceId', authenticateToken, async (req, res) => {
    try {
        ...
        try {
            const timeRangeStart = timeRange.start.replace(':', '.');
            const timeRangeEnd = timeRange.end.replace(':', '.');
            const customString = `${timeRangeStart}:${timeRangeEnd}:${
                Number(frequency) * 1000
            }`; // format:  HH.mm:HH.mm:ms   start:end:frequency

            // Send the custom string to the Particle device, so it knows to update
            const response = await axios.post(
                `https://api.particle.io/v1/devices/${particle_id}/storeString`,
                new URLSearchParams({
